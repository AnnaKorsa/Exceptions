...
public class Main {
    ...
    public static String inputData() {
        System.out.println(
                ...
                "<Фамилия> <Имя> <Отчество>(В ФИО должны быть только буквы латинского алфавита не более 20 букв в каждом слове!)\n" +
                " <Дата рождения в формате dd.mm.yyyy>\n" +
                " <Номер телефона(только цифры до 15 символов)>\n " +
                "<Пол: f - жен. или m - мужской>\n" +
                "\n!!! ВАЖНО: фамилия должна быть введена раньше чем имя, а имя раньше чем отчество!\n");
        String input = "";
    ...
    public static String[] checkLengthData(String input) {
        ...
        if (result.length > dataLenght) {
            throw new LengthInputException("Ошибка! Вы ввели больше 6 параметров: " + input);
        }
        if (result.length < dataLenght) {
            throw new LengthInputException("Ошибка! Вы ввели меньше 6 параметров: " + input);
        }
        return result;
    }
    ...
    public static void writeData(String[] data) {
        ...
        try (FileWriter writer = new FileWriter(String.format("C:\\Users\\User\\Новая папка\\Java\\HW2\\src\\HW3\\%s.txt",fileName),
                true)) {
